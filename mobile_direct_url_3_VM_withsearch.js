...
(async () => {
    ...
    const timeout = 90000; // Increased timeout to allow more loading time
...
await lhFlow.endNavigation();
...

    {
		const searchtab = await Promise.race([
            page.waitForSelector('::-p-aria(Search)'),
            page.waitForSelector("[data-testid='global-search']"),
            page.waitForSelector('::-p-xpath(//*[@data-testid=\\"global-search\\"])'),
            page.waitForSelector(":scope >>> [data-testid='global-search']")
        ])
		await searchtab.click('::-p-aria(Search)');
    }
    ...
    await lhFlow.startNavigation();
    {
		const searchpage = await Promise.race([
            ...
            page.waitForSelector('::-p-text(Search for \\"Pepsi)')
				...
				await searchpage.click('::-p-aria(Search for \\"Pepsi Cola 480ml PET Bottle x24\\") >>>> ::-p-aria([role=\\"generic\\"])');
...
//                    page.waitForSelector('::-p-aria(Clear the cart) >>>> ::-p-aria([role=\\"generic\\"])'),
	...
	await new Promise(resolve => setTimeout(resolve, 2000));
           ...
           // await Promise.all(promises);
            ...
            fs.writeFileSync(`./Report/SIT/19May_Mobile_place_order/flow_${i + 1}.report.html`, lhFlowReport);
            ...
            fs.writeFileSync(`./Report/SIT/19May_Mobile_place_order/flow_${i + 1}_failure.report.html`, failureReport);
